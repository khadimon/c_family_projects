...
compile: mpicc -O2 -o g1 group1.c -lm
run: mpirun -c <processes> ./g1 
*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
char IMG[] = "IMG/image.ppm"; //Choose what image you want to process, it has to be ppm
...
int main(){
    ...
    unsigned char *newData = (unsigned char *) malloc(size * sizeof(unsigned char) /comm_sz);
    ...
    newData = Sobel(my_rank, comm_sz, data);

    split = size/comm_sz; 

    /*send data to process 0*/
    if (my_rank != 0) MPI_Send(newData, split, MPI_UNSIGNED_CHAR, 0, 0, MPI_COMM_WORLD);
        ...
        for (i = 1; i < comm_sz; i++) {
            MPI_Recv(newData, split, MPI_UNSIGNED_CHAR, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); /*Recieve sum from each process*/
            for(j = 0; j < size/comm_sz; j++){
